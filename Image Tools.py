...
class ImageToolsMod(loader.Module):
    """Модуль инструментов изображения
    Автор Узбек🇺🇿🤝🇺🇦 @the_farkhodov """
    ...
    async def soapcmd(self, message):
        ...
        if message.is_reply:
            ...
            if isinstance(data, bool):
                await utils.answer(message, "<code>Реплэй на фото или стик!</code>")
                return
        else:
            await utils.answer(message, "<code>Реплэй на фото или стик!</code>")
            return

        await message.edit("намыливание ...")
    ...
    async def pic2packcmd(self, message):
        """Создайте набор наклеек с вашей фотографией """
        reply = await message.get_reply_message()
        if not reply:
            await message.edit("<b>Реплэй на фото❗</b>")
            return

        args = utils.get_args_raw(message)
        if not args:
            await message.edit("<b>Название стикерв</b>❓")
            return
        chat = "@Stickers"
        name = "".join(
        ...
        w, h = image.size
        www = max(w, h)
        await message.edit("🔪<b>Обрезание...</b>")
        ...
        face.save(fface)
        fface.seek(0)
        await message.edit("<b>📤Загружается...</b>")
            ...
            try:
                ...
                for part in ending.raw_text.split():
                    if part.startswith("https://t.me/"):
                        break
                await message.edit("✅<b>Загрузка завершена успешно!</b>\n" + part)
